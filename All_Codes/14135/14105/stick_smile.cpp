...
void display() {
  ...
  glLoadIdentity();
  glTranslatef(-0.28f,1.85f,-6.0f);
  glColor3f(1.0f,0.0f,0.0f);
  glBegin(GL_POINTS);
  for(int i=0;i<1000;++i)
    {
      glVertex3f(cos(2*3.14159*i/1000.0)*0.05,sin(2*3.14159*i/1000.0)*0.06,0);
    }
  glEnd();

  glLoadIdentity();
  glTranslatef(-0.09f,1.85f,-6.0f);
  glColor3f(1.0f,0.0f,0.0f);
  glBegin(GL_POINTS);
  for(int i=0;i<1000;++i)
    {
  ...
  for(int i=0;i<1000;++i)
    {
      if(i>=600 && i<=900)
      glVertex3f(cos(2*3.14159*i/1000.0)*0.2,sin(2*3.14159*i/1000.0)*0.2,0);
    }
  glEnd();
